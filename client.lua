...
AddEventHandler('ug_militaryJob_backup:BackupReq', function(bk, s, playerName)
    ...
    if PlayerData.job.name == Config.jobname then
        if bkLvl == "1" then
            bkLvlTxt = "~g~Low Backup"
        elseif bkLvl == "2" then
            bkLvlTxt = "~y~Medium Backup"
        elseif bkLvl == "3" then
            bkLvlTxt = "~r~Hard Backup"
            PlaySoundFrontend(-1, "Mission_Pass_Notify", "DLC_HEISTS_GENERAL_FRONTEND_SOUNDS", 1)
        elseif bkLvl == "99" then
            bkLvlTxt = "~r~~h~Air Strike"
        end
        ShowInfo("The Military ~g~" ..  playerName .. "~w~ Is Requesting An " .. bkLvlTxt .. "~s~. ~o~At: ~s~" .. streetName .. ".")
